...
def cross_train(jdata, ndata, jtabel, ntabel, T, train_method, public_key, private_key):  # 交叉验证法测试
    iter_num = 0  # 迭代次数总和
    TPsum = 0  # 预测正例中正例的个数
    FNsum = 0  # 预测正例中反例的个数
    FPsum = 0  # 预测正例中正例的个数
    ...
    for j in range(T):
        ...
        for i in range(2):
            ...
            omega, b, t, DPtime, DACtime = train_method(train_x, train_y, train_num, public_key, private_key)
            end_time = time.time()
            run_time += end_time - begin_time
            DPtime_sum += DPtime
            DACtime_sum += DACtime
            print("训练耗时：%f, DP耗时：%f, DAC耗时：%f" % (end_time - begin_time, DPtime, DACtime))
            iter_num += t
            # Ex = 0  # 正例的个数
            # Count = 0  # 反例的个数
            test_num = len(test_y)
